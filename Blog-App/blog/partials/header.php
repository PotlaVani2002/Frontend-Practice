...
// Fetch current user from the database if logged in
if (isset($_SESSION['user-id'])) {
    $id = filter_var($_SESSION['user-id'], FILTER_SANITIZE_NUMBER_INT);
    
    // Query to fetch user avatar
    $query = "SELECT avatar FROM users WHERE id=$id";
    $result = mysqli_query($connection, $query);
    
    // Check if the query was successful
    if ($result && mysqli_num_rows($result) == 1) {
        $avatar = mysqli_fetch_assoc($result);  // Fetch the avatar field
    } else {
        $avatar = null;  // Set to null if user is not found
    }
} else {
    $avatar = null; // If user is not logged in
}
?>
